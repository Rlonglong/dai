...
> 📌 這個資料夾**有自己的 GitLab repo（`bcp-scripts`）**，跟 `dagster_workspace` 是兩條獨立的
> CI/CD 線，合併進 `main` 後由 CD 用 rsync 推到 VM1。
> **不要直接在 VM1 上改檔案**——下次同步會被蓋掉，而且每日 hash 對帳會告警。
> 見 [GitLab 維運手冊](../../gitlab_workspace/GitLab維運手冊/README.md)。
